...
citi %>% 
  mutate(weekends = as.factor(weekdays)) %>% 
  arrange(desc(weekends)) %>% 
  summarize(median_count = median(duration)) %>% 
  ggplot(aes(x = weekdays, y = median_count), fill = month) +
  geom_histogram(aes(size = median_count)) +
  facet_grid(. ~ year) +
...
citi_2024 %>%
  ggplot(aes(x = duration, fill = interaction(month, rideable_type, member_casual))) +
  geom_histogram(position = "dodge") +
  facet_grid(. ~ month) +
...
There is a higher duration of rides in July compared to January, which can be seen by comparing the two graphs. In January, there is a higher number of classic bike members, whereas in July, there is a higher number of electric bike members. In both months, the peak of ride duration was around 15miles, which few individuals having a ride duration of over 50 miles.